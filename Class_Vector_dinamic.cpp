...
Vector Vector::eq_operation(Vector& other) {
    Vector res = this->get_res_vector(other);
    for (int i = 0; i < in_use; i++) {
...
Vector Vector::more_operation(Vector& other) {
    Vector res = this->get_res_vector(other);
    for (int i = 0; i < in_use; i++) {
...
Vector Vector::more_eq_operation(Vector& other) {
    Vector res = this->get_res_vector(other);
    for (int i = 0; i < in_use; i++) {
...
Vector Vector::less_operation(Vector& other) {
    Vector res = this->get_res_vector(other);
    for (int i = 0; i < in_use; i++) {
...
Vector Vector::less_eq_operation(Vector& other) {
    Vector res = this->get_res_vector(other);
    for (int i = 0; i < in_use; i++) {
...
Vector Vector::comparison (Vector& other, const string& op) {
...
string Vector::get_string_array() const{
    string str(array);
    return str;
}
...
ostream &operator << (ostream &os, const Vector& v){
    return os << v.get_in_use() << endl << v.get_string_array();
}
